...
| `G2M` | `scripts/go2_mjlab_dreureka_port/runner.py` | Implements MJLab port evidence generation, 4096-env task smoke, 4096-env Isaac-like terrain assignment verification, reset-scene rendering, training launch orchestration without shrinking terrain rows/cols, TensorBoard reward extraction, same-iteration baseline comparison, and health reports for smoke and 1/8-budget runs. |
...
| `G2M` | `scripts/go2_mjlab_dreureka_port/dreureka_go2_mjlab/mdp/events.py` | Implements script-local DrEureka reset, Isaac-like startup terrain-origin assignment, robot-only `+-0.05m` XY reset jitter, and robot/ball-scoped pretrained-style DR: ball radius/mass/inertia/friction observations, robot friction/payload/COM/motor offset/strength, action lag, pushes, gravity observation offset, and ball drag without terrain material randomization or custom MuJoCo contact-solver assignments. |
