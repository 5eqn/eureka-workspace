...
| `G2P` | `artifacts/go2_yoga_ball/latest_dr_eureka_8_isaacgym_playback/` | Isaac-side playback bundle for the same eight newest 2026-06-03 DrEureka run directories, using each run's saved `parameters.pkl` contract and saved DR ranges; includes eight validated 600-frame MP4s, `manifest.json`, and `report.md` showing the saved contracts are Go1 constrained-URDF actuator-net policies rather than Go2 PD policies. |
...
| `G1T` | `logs/dr_eureka_rapp_checkpoint_review/` | Raw stock RAPP-mode Isaac Gym playback logs for the three checkpoint-review videos, including `z-position` lines used to compute each manifest height result and the teardown segfault evidence after MP4 writing. |
| `G1T` | `logs/dr_eureka_rapp_0p8/` | Relaxed `0.8m` globe-walking RAPP console logs: `near_miss_142042_rapp.log` completed all parameter sweeps and emitted the current prompt bounds, while `best_213133_rapp.log` failed before finishing at `robot_com_displacement_range`. |
...
| `G2P` | `logs/go2_yoga_ball/latest_dr_eureka_8_isaacgym_playback/` | Raw Isaac Gym playback logs for the eight newest 2026-06-03 DrEureka run directories under saved-contract playback, including playback CSVs, summaries, process-status records accepting the known post-video graphics teardown segfault only after artifact completion, and source MP4s copied into the artifact bundle. |
...
| `S` | `scripts/go2_yoga_ball/isaacgym_playback_smoke.py` | Executes Isaac Gym playback and writes playback CSV/report/video artifacts, with the default corrected Go2 PD playback path plus explicit saved-contract/saved-DR flags used to audit older DrEureka checkpoints without silently rewriting their `parameters.pkl` robot/control contract. |
...
| `G2M` | `scripts/go2_mjlab_dreureka_port/AGENTS.md` | Folder-local rule that this Go2 MJLab port must prioritize the server runtime with MJLab 1.0.0 and legacy RSL-RL because that compute architecture cannot upgrade MJLab for local convenience. |
...
| `G2M` | `scripts/go2_mjlab_dreureka_port/play_driver.py` | Loads the caller-project DrEureka Go2 MJLab task and a saved training checkpoint into MJLab's MuJoCo viewer through the low-version MJLab/RSL-RL runner API, including actor-state translation for split actor/critic checkpoints, so server playback does not depend on newer `load_cfg` APIs. |
| `G2M` | `scripts/go2_mjlab_dreureka_port/MUSA_COMPATIBILITY.md` | Documents that this port is server-first, pins the intended lambdalab/MJLab `v1.0.0` and `rsl-rl-lib==3.1.0` runtime shape, and records the server-side MJLab-to-lambdalab rename, `OnPolicyRunner` fallback, and `MUSA_VISIBLE_DEVICES`/`torchrunx` launch contract without committing server-only package-name edits. |
| `G2M` | `scripts/go2_mjlab_dreureka_port/train_driver.py` | Runs the registered MJLab DrEureka Go2 task with registry-runner support plus `rsl_rl.runners.OnPolicyRunner` fallback, CLI-controlled env count and training budget, optional terrain rows/cols overrides, seed/launch JSON recording, MuJoCo solver/contact settings, generic CUDA/MUSA `torchrunx` multi-worker launch, and transparent MJLab `select_gpus("all")` device selection. |
| `G2M` | `scripts/go2_mjlab_dreureka_port/test_train_driver_cfg.py` | Smoke-tests that `train_driver.py` passes the server-required legacy `policy=RslRlPpoActorCriticCfg(...)` runner config through unchanged instead of converting it to the newer RSL-RL `actor`/`critic` shape. |
...
| `G2M` | `scripts/go2_mjlab_dreureka_port/dreureka_go2_mjlab/rl_cfg.py` | Holds the server-first lambdalab/MJLab `v1.0.0` RSL-RL `3.1.0` runner settings for the baseline reproduction, including legacy `policy` observation groups, critic inputs `actor+critic`, 20000 iterations, save interval 1000, and tensorboard logging. |
