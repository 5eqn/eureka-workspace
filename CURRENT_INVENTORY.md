...
- `G2M` Go2 MJLab Port: FRESH host-side MJLab reproduction has a self-contained caller-project task using Unitree RL MJLab Go2 PD gains, MJLab term-major actor history with DrEureka/Isaac-side sensor terms, an RSL-RL critic input of actor history plus 11 privileged values, MJLab built-in `HfPerlinNoiseTerrainCfg` over Isaac-like 20x20 terrain origins with 5m tiles, 0.25m hfield division, Perlin coordinate scale 20, roughness range 0.02..0.08, fixed terrain-origin assignment over rows/cols 5..14 with robot XY reset jitter, and robot/ball-scoped pretrained-style DrEureka domain randomization; 4096-env task smoke and 4096-env smoke training pass without hfield collision-overflow warnings.
...
| `G2M` | `artifacts/go2_mjlab_dreureka_port/source_contract.json` | Machine-readable source contract comparing DrEureka Go2 yoga-ball requirements with home-space MJLab/Unitree RL MJLab sources and the caller-project port using MJLab built-in Perlin terrain at 20x20 5m-tile layout, 0.25m division, scale 20, and 0.02..0.08 height range. |
| `G2M` | `artifacts/go2_mjlab_dreureka_port/source_contract.md` | Human-readable source contract naming the Go2 robot, yoga-ball task, reward/observation/termination contract, built-in Perlin terrain requirement, fixed terrain-origin reset semantics, and 1/8-budget semantics. |
| `G2M` | `artifacts/go2_mjlab_dreureka_port/task_config_smoke.json` | Machine-readable proof that `DrEureka-Go2-YogaBall` registers and steps 4096 envs on CUDA with 20x20 `hf_perlin_noise` terrain tiles of size 5m, compiles 400 hfields, assigns terrain origins inside rows/types 5..14, uses Unitree RL MJLab hip/thigh/calf PD gains, exposes MJLab term-major 56x15 actor observations, builds the RSL-RL critic from actor history plus an 11-D privileged group, removes velocity commands, and samples pretrained ball/friction/restitution/motor/lag ranges. |
| `G2M` | `artifacts/go2_mjlab_dreureka_port/task_config_smoke.md` | Human-readable 4096-env task-config smoke summary for the corrected Unitree-PD, built-in Perlin MJLab Go2 yoga-ball task registration, terrain-origin assignment check, one-step runtime check, and actor/critic model input dimensions. |
| `G2M` | `artifacts/go2_mjlab_dreureka_port/smoke_20min_launch.json` | Launch record for the 4096-env MJLab PPO smoke training run using built-in Perlin terrain with 0.25m division and scale 20. |
| `G2M` | `artifacts/go2_mjlab_dreureka_port/smoke_20min_health.json` | Machine-readable health report proving the 4096-env built-in-Perlin smoke training run completed, produced finite nonzero reward data, and had no hfield collision-overflow warning, traceback, or NaN text. |
| `G2M` | `artifacts/go2_mjlab_dreureka_port/smoke_20min_health.md` | Human-readable health summary for the 4096-env built-in-Perlin smoke training run. |
| `G2M` | `artifacts/go2_mjlab_dreureka_port/smoke_20min_reward_curve.csv` | TensorBoard-derived reward curve for the 4096-env built-in-Perlin smoke training run. |
| `G2M` | `artifacts/go2_mjlab_dreureka_port/smoke_20min_reward_curve.svg` | Plot of the 4096-env built-in-Perlin smoke training reward curve. |
...
| `G2M` | `logs/go2_mjlab_dreureka_port/smoke_20min/rsl_rl/events.out.tfevents.1780146655.isaac.389494.0` | TensorBoard event file from the 4096-env built-in-Perlin smoke training run used to generate the current reward curve. |
| `G2M` | `logs/go2_mjlab_dreureka_port/smoke_20min/rsl_rl/git/eureka-workspace.diff` | RSL-RL snapshot of the workspace diff captured during the 4096-env built-in-Perlin smoke training run. |
| `G2M` | `logs/go2_mjlab_dreureka_port/smoke_20min/rsl_rl/model_0.pt` | Initial checkpoint emitted by the 4096-env built-in-Perlin smoke training run. |
| `G2M` | `logs/go2_mjlab_dreureka_port/smoke_20min/rsl_rl/model_1.pt` | Second checkpoint emitted by the 4096-env built-in-Perlin smoke training run, proving the smoke ran beyond startup. |
| `G2M` | `logs/go2_mjlab_dreureka_port/smoke_20min/train.log` | Console log for the 4096-env built-in-Perlin smoke training run; checked for no hfield collision-overflow warning, no traceback, and no NaN text. |
...
| `G2M` | `scripts/go2_mjlab_dreureka_port/run.sh` | Entry point for the FRESH MJLab Go2 port caller project: preflight, source contract, environment/import/task smokes, 4096-env smoke training, 1/8-budget training, report generation, and local MuJoCo viewer playback. |
| `G2M` | `scripts/go2_mjlab_dreureka_port/runner.py` | Implements MJLab port evidence generation, 4096-env training launch orchestration, TensorBoard reward extraction, same-iteration baseline comparison, and health reports for smoke and 1/8-budget runs. |
...
| `G2M` | `scripts/go2_mjlab_dreureka_port/dreureka_go2_mjlab/env_cfg.py` | Defines the MJLab Go2 yoga-ball task: Unitree Go2 robot data with DrEureka pose/control overrides, ball entity, command-free DrEureka/Isaac-side actor terms flattened by MJLab's term-major history convention, privileged critic terms, Eureka rewards, body-height terminations, built-in `HfPerlinNoiseTerrainCfg` terrain at 20x20 5m-tile layout with 0.25m division, scale 20, 0.02..0.08 height range, Isaac-like fixed terrain-origin assignment over rows/cols 5..14, pretrained DR ranges, and 4096-env baseline settings. |
