...
| `G2M` | `scripts/go2_mjlab_dreureka_port/MUSA_COMPATIBILITY.md` | Documents the server-side MJLab-to-lambdalab rename, MJLab `v1.0.0` no-deps runtime constraint, `OnPolicyRunner` fallback, and `MUSA_VISIBLE_DEVICES`/`torchrunx` launch contract for moving the verified local training port to the MUSA cluster without committing server-only package-name edits. |
